...
window.addEventListener("load", function load(event){
	...
	map = L.map("map").setView([47.21725, -1.55336], 12);
	/*L.tileLayer(
...
function loadData() {
	...
	var center3857 = proj4('EPSG:3857', 'EPSG:2154', [center.lng, center.lat]);
	var ne3857 = proj4('EPSG:3857', 'EPSG:2154', [ne.lng, ne.lat]);

	console.log(center3857)

	var data = {
		x : 6688724, //center3857[0],
		y : 354684, //center3857[1],
		rayon : 50000,//*Math.sqrt(Math.pow(center3857[0] - ne3857[0], 2) + Math.pow(center3857[1] - ne3857[1], 2))
	};
	...
	$.ajax({
		...
		success : function(json) {
			...
			$.each(data.stations, function(i, station) {
				station_marker = L.circleMarker(station.pos, {size: 5, color: getColorByStatus(station.status)}).addTo(map);
